...
class SaleOrderLine(models.Model):
    ...
    pack_total = fields.Float(string='Pack total', compute='_get_pack_total')
    pack_line_ids = fields.One2many('sale.order.line.pack.line', 'order_line_id', 'Pack Lines')
    pack_type = fields.Selection(related='product_id.pack_price_type', readonly=True)

    # Fields for common packs
    pack_depth = fields.Integer('Depth', help='Depth of the product if it is part of a pack.')
    pack_parent_line_id = fields.Many2one('sale.order.line', 'Pack', help='The pack that contains this product.',
                                          ondelete="cascade")
    pack_child_line_ids = fields.One2many('sale.order.line', 'pack_parent_line_id', 'Lines in pack')
    ...
    def expand_pack_line(self):
        ...
        if self.state == 'draft' and self.product_id.pack and self.pack_type in detailed_packs:
            for subline in self.product_id.pack_line_ids:
                ...
                domain = [('product_id', '=', subline.product_id.id), ('pack_parent_line_id', '=', self.id)]
                existing_subline = self.search(domain, limit=1)
    ...
    def action_pack_detail(self):
        if self.pack_type != 'none_detailed_assited_price':
            raise UserError(_('Action unavailable for this line'))
        view_id = self.env['ir.model.data'].xmlid_to_res_id('product_pack.view_order_line_form2')
        view = {
            'name': _('Details'),
            'view_type': 'form',
            'view_mode': 'form',
