...
class ProductPack(models.Model):
    ...
    quantity = fields.Float('Quantity', required=True, default=1.0, digits=dp.get_precision('Product UoS'))
    ...
    def get_sale_order_line_vals(self, line, order):
        ...
        # if pack is fixed price or totlice price we don want amount on pack lines
        if line.product_id.pack_price_type in ['fixed_price', 'totalice_price']:
            price = 0.0
            discount = 0.0
        else:
            pricelist = order.pricelist_id.id
            context = {'uom': subproduct.uom_id.id, 'date': order.date_order}
            pricelist_obj = self.env['product.pricelist']
            price = pricelist_obj.price_get(subproduct.id, quantity, order.partner_id.id, context=context)[pricelist]
            discount = self.discount

        # Obtain product name in partner's language
        if order.partner_id.lang:
            subproduct = subproduct.with_context(lang=order.partner_id.lang)
